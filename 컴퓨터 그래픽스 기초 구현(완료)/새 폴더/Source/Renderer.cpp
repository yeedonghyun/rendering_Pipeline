			...
			for (k = xMin; k < xMax; k++) {
				if (AET[j].z + deltaZ < g_renderer.zDepthBuffer[i][k][0])
				{
					...
					if (isLighted) {
						...
						//checkImage[i][k][0] = (GLubyte)(textureImage[(int)(AET[j].v + deltaV)][(int)(AET[j].u + deltaU)][0]);
						//checkImage[i][k][1] = (GLubyte)(textureImage[(int)(AET[j].v + deltaV)][(int)(AET[j].u + deltaU)][1]);
						//checkImage[i][k][2] = (GLubyte)(textureImage[(int)(AET[j].v + deltaV)][(int)(AET[j].u + deltaU)][2]);
